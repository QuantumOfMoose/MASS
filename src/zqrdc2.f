...
c       avoid accessing element beyond the bound
         do 70 j = 1, p
            ...
            work(j,1) = qraux(j)
            work(j,2) = qraux(j)
            if(cabs1(work(j,2)) .eq. 0.0d0) work(j,2) = dcmplx(1.0d0,0.0d0)
  70     continue
      endif
c
...
c
   80    continue
         if (l .ge. k .or. dreal(qraux(l)) .ge. dreal(work(l,2)*tol)) go to 120
            lp1 = l+1
            do 100 i=1,n
               t = x(i,l)
               do 90 j=lp1,p
